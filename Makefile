...
# Targets
TARGETS = .vt .perl .vep .exac .maxentscan .egenome .picard .htslib .bwa .samtools .bcftools .freebayes .bamStats src/nRex

all:   	$(TARGETS)

.vt:
	cd src/vt/ && make && cd ../../ && touch .vt
...
.freebayes: $(FREEBAYSOURCES)
	cd src/freebayes && make && cd vcflib && cat Makefile | egrep -v -f <(grep "Fasta.h" src/*.cpp | sed 's/:.*$//') > Makefile.tmp && mv Makefile.tmp Makefile && make all && cd ../../../ && touch .freebayes
