...
.tools: .mamba
	export PATH=${PBASE}/mamba/bin:${PATH} && mamba install -y -c conda-forge -c bioconda samtools bcftools bedtools htslib bwa trim-galore fastqc delly alfred freebayes wally minimap2 && pip install nanopack && touch .tools

.check: .mamba .tools
	export PATH=${PBASE}/mamba/bin:${PATH} && samtools --version && bcftools --version && bedtools --version && bgzip --version && tabix --version && trim_galore --version && delly --version && echo "Installation complete!" && touch .check
